...
interface IErrorsEvents is IErrorFieldProvider {
    ...
    // Emitted when the fee of a kopio is updated.
    event FeeSet(
    ...
    // Emitted when global config of kopio is updated.
    event SCDPKopioUpdated(
        address indexed _asset,
        uint256 openFee,
        uint256 closeFee,
        uint256 protocolFee,
    ...
    /**
     * @notice Emitted on updates to global liquidation incentive of a kopio.
     * @param symbol target symbol
     * @param asset target kopio.
    ...
    /**
     * @notice Emitted when global MCR is updated.
    ...
    /**
     * @notice Emitted when account mints new assets.
     * @param account account that minted.
     * @param kopio address of kopio.
    ...
    /**
     * @notice Emitted when an account burns a kopio.
     * @param account The address of the account burning the kopio.
     * @param kopio The address of the kopio.
     * @param amount The amount of the kopio burned.
     */
    ...
    /**
     * @notice Emitted when an account pays an open/close fee in the Minter.
     * @dev This can be emitted multiple times for a single asset.
     * @param account Account paying the fee.
     * @param feeAsset Asset used to pay the fee.
     * @param feeType Fee type.
     * @param paymentAmount Amount paid.
     * @param paymentValue USD value.
     */
    event FeePaid(
        address indexed account,
        address indexed feeAsset,
    ...
    /**
     * @notice Emitted when a liquidation occurs in the Minter.
     * @param account account being liquidated.
     * @param liquidator account performing the liquidation.
     * @param repayKopio kopio repaid by the liquidator.
     * @param repayAmount amount repaid.
     * @param seizeCollateral collateral seized from the account.
     * @param seizeAmount amount seized
     */
    ...
    error INVALID_KOPIO_CONTRACT(ID);
    error INVALID_FIXED_KOPIO_CONTRACT(ID fixedKopio, ID kopio);
    error NOT_SWAPPABLE_KOPIO(ID);
    ...
    error COLLATERAL_AMOUNT_LOW(ID collateral, uint256 amount, uint256 min);
    ...
    error SENDER_NOT_OPERATOR(ID, address sender, address protocol);
